...
Live in the quizzes: 2026-03-02.

\begin{enumerate}
    \item Consider $MA(1)$ process $y_t = 5 + u_t + 2u_{t-1}$ where $(u_t)$ is a white noise process.
    \begin{enumerate}
        \item Find $\E(y_t)$.
        \item Find the autocorrelation function $\rho_k$.
        \item Find the first two values of the partial autocorrelation function, $\phi_{11}$, $\phi_{22}$.
    \end{enumerate}

    \item Consider $MA(2)$ process $y_t = 5 + u_t + 2u_{t-1} + 3u_{t-2}$ where $(u_t)$ is a white noise process.
    \begin{enumerate}
        \item Find $\E(y_t)$.
        \item Find the autocorrelation function $\rho_k$.
        \item Find the first two values of the partial autocorrelation function, $\phi_{11}$, $\phi_{22}$.
    ...
    \item Consider the stationary $AR(1)$ process with equation $y_t = 5 + 0.3 y_{t-1} + u_t$ where $(u_t)$ is a white noise process.
    \begin{enumerate}
        \item Find $\E(y_t)$.
        \item Find the autocorrelation function $\rho_k$.
        \item Find the first two values of the partial autocorrelation function, $\phi_{11}$, $\phi_{22}$.
...
Live in the quizzes: 2026-03-23.

\begin{enumerate}
\item Variables $y_1$, $y_2$, \dots, $y_n$ are independent and identically distributed with $\E(y_i^3) = \sqrt{a - 7}$,
where $a$ is an unknown parameter.

Estimate $a$ using the method of moments. 

\item Variables $y_1$, $y_2$, \dots, $y_n$ are independent and identically distributed with density $f(y) = (a + 0.5) y^{a- 0.5}$ for $y \in [0;1]$
where $a$ is an unknown parameter.

Estimate $a$ using maximum likelihood. 
\end{enumerate}
